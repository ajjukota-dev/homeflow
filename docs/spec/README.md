...
| Layer | Choice |
|---|---|
| Base | **HomeFlow v1** (`Pranava-V2/HomeFlow`) — carried, not rewritten. See [`foundation/v1-reuse.md`](foundation/v1-reuse.md) and [`00-REVIEW.md`](00-REVIEW.md). |
| Frontend | **React** + Vite, TypeScript — v1's workspace (migrated from CRA) + the customer portal |
| Backend | **Python / FastAPI** (v1's), one container on AWS ECS — see [`foundation/architecture.md`](foundation/architecture.md) |
| API | REST + JSON, versioned (`/api/v1`), OpenAPI generated by FastAPI |
| Data | PostgreSQL 16 (RDS in AWS, Docker locally) as system of record, RLS by project; event log append-only by grant; jobs/sessions in the same DB. Migrated from v1's MongoDB. |
| Files | S3 (MinIO locally), signed URLs |
| Auth | Staff: Google OIDC server-side (replaces v1's Emergent broker). Customers: OTP on the booking mobile. Sessions in Postgres; no passwords stored. Role/project scope via RLS |
...
```
docs/spec/
├── 00-REVIEW.md                  ← design-choice review (5 Sep 2026) — read before building
...
│   ├── architecture.md           ← target architecture (Fargate + RDS Postgres + S3; jobs and sessions in Postgres)
│   └── v1-reuse.md               ← ★ what is carried from v1, what is refactored, how data migrates
├── technical/                    ← ★ how it is built: 01 backend layout · 02 database/RLS · 03 identity · 04 events+jobs
│                                    05 action+journey · 06 engines · 07 API map · 08 files/docs/notify · 09 frontend
│                                    10 infra+delivery · 11 migration runbook · 12 testing+NFR  (read after foundation)
...
**Build order:** **migrate v1** ([`foundation/v1-reuse.md`](foundation/v1-reuse.md) §5: freeze → import → Postgres + RLS → data + files → port engines → event log → Action → identity) → then `project-site` → `sales` → `crm-rm` → (`accounts` ‖ `legal` ‖ `qa`) → `post-handover` → `customer` → `management`. Every v1 screen must render from Postgres before any new feature lands.
...
| Module | Spec status |
|---|---|
| `foundation` (13 files incl. transparency surface + v1-reuse) | ✅ complete — architecture revised 5 Sep 2026 |
