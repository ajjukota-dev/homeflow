# HomeFlow OS — Customer Twin, Unit Twin, and the villa operating system

Source of this document: `Pranava_HomeFlow_2.0_Full_Design_Spec_v8.pdf` (48 pages, 35 sections + 2 appendices), transcribed from the interactive canvas [`canvases/homeflow-2-design-spec.canvas.tsx`](../canvases/homeflow-2-design-spec.canvas.tsx). Short briefing: [`docs/CONTEXT.md`](CONTEXT.md).

**This is one product.** HomeFlow is Pranava’s residential post-sales operating system for villas and apartments. It is not the office-leasing / FMWork product. East Crest is an example Project — configuration, not a code branch.

---

## What we are building

Pranava takes a **Project** (example: East Crest), builds **villas or apartments**, and sells those **Units** to families.

HomeFlow is the software that keeps **the customer** and **that exact home** as two digital twins, then joins them with a **Booking** so Sales, CRM, site, accounts, legal, and management work from one story:

- **Before anyone buys** — the Unit Twin already exists. Sales can see whether this villa can still change the kitchen, electrical, flooring.
- **When they book** — a Customer Twin is linked to that Unit Twin through a Booking.
- **Until keys and after** — money, papers, construction of that unit, customer changes, registration, handover, warranty, and Home Passport stay on those twins.

HomeFlow is not the construction drawing tool and not the accounting ledger. It is the OS that keeps the customer, the unit, and the money in one story.

Design target: premium customer experience + disciplined execution + controlled transparency + profitable growth.

| | |
|---|---|
| 2 twins | Customer + Unit |
| 11 stages | Generic lifecycle (0–10) |
| 14 | Functional modules |
| 5 gates | Changeability states |
| 9 P0 | 0–90 day foundation |
| 5 tests | Before any feature ships |

**System independence.** HomeFlow must be fully operable alone. No construction, ERP, CRM, finance, DMS or FM platform is assumed. Project teams maintain physical truth inside HomeFlow. Later connectors are optional accelerators — they must never dictate the domain model.

---

## Contents

1. [The OS in one sentence](#1-the-os-in-one-sentence)
2. [Three things that never mix](#2-three-things-that-never-mix)
3. [How a villa actually moves](#3-how-a-villa-actually-moves)
4. [Who opens what](#4-who-opens-what)
5. [Keywords](#5-keywords)
6. [What 2.0 must achieve](#6-what-20-must-achieve)
7. [Anatomy — twins and data model](#7-anatomy--twins-and-data-model)
8. [Journey — 11 stages](#8-journey--11-stages)
9. [Changeability and Change Requests](#9-changeability-and-change-requests)
10. [14 modules](#10-14-modules)
11. [Operate — scores, actions, My Day, Control Tower](#11-operate--scores-actions-my-day-control-tower)
12. [Money](#12-money)
13. [CX + AI](#13-cx--ai)
14. [Ship — roadmap and what not to build](#14-ship--roadmap-and-what-not-to-build)
15. [Emergent build — implementation contracts](#15-emergent-build--implementation-contracts)
16. [Reference — statuses, events, screens, integrations, policy](#16-reference--statuses-events-screens-integrations-policy)
...
## 2. Three things that never mix

```
Customer Twin  ——  Booking (this ownership period only)  ——  Unit Twin
                      │
                      ▼
         Project → Phase / Tower / Block → Floor → Unit
```

| Thing | Meaning |
|---|---|
| **Project** | The site (East Crest, or any other). Land + towers/villas + teams. All reporting rolls up here. |
| **Unit** | The physical home (Villa V104 or Flat 8-12). Exists **before** anyone buys it. Has walls, wiring, flooring progress. Keeps a permanent history even if the buyer changes. |
| **Booking** | This family + this unit + this ownership period. Cancel or transfer? Booking closes. Unit stays. |
| **Customer** | The person/family. Relationship, money behaviour, docs, promises, comms. |

Attach commercial and lifecycle facts to the **Booking**, not directly to the customer or the unit. That is what survives cancellation, transfer, joint-ownership change and resale.

Sales may **read** changeability gates. Sales never **edits** unit physics.

---

## 3. How a villa actually moves

| When | What happens in real life | What HomeFlow does |
|---|---|---|
...
| Person | They open HomeFlow and should immediately know |
|---|---|
...
| Keyword | Meaning |
|---|---|
| Project-first | Every booking, unit, collection, CR, handover, task and forecast resolves to a Project. |
| Unit Digital Twin | Live physical truth of a unit — even before it is sold. Progress, gates, as-built, passport. |
| Customer Digital Twin | Relationship record: money, docs, commitments, comms, sentiment, consent. |
| Booking | The bridge. Attach commercial/lifecycle facts here so cancel/transfer/resale does not corrupt history. |
| System independence | HomeFlow must run standalone. External CRM/ERP/construction systems are optional adapters. |
| Changeability | Per-category gate derived from live unit physics, not booking date or a project-wide cutoff. |
| OPEN / CLOSING / CONDITIONAL / EXCEPTION / HARD CLOSED | The five gate states Sales and CRM may read but never edit. |
| Change Window Hold | Time-boxed, Project-approved pause so a serious sale can keep a closing option open. |
| Universal Action | Every task, snag, approval, gap, delay and AI rec normalizes into one Action object. |
| My Day | Employees do not search for work. HomeFlow ranks what matters today and why. |
| Promise Ledger | Every Pranava promise and every customer promise-to-pay, with owner, due date, evidence. |
| Hard gates | Handover cannot proceed without named authority + reason. Safety/statutory gates never override. |
| Explainable scores | Unit / Customer / Handover / Health / Financial — each answers a different question, with drivers. |
| Legal Document Factory | Governed generation from approved templates + clause library. Not free-form mail merge. |
| True-risk collections | Split outstanding vs due vs overdue vs disputed vs loan-dependent vs promise-to-pay vs true risk. |
| Forecast snapshot | Month-start forecast is immutable. Revisions are new versions. Never overwrite history. |
| My Pranava Home | Customer sees a calm journey. Never internal blame, vendor disputes, or unapproved forecasts. |
| Control Tower | Management sees five ranked interventions, not fifty charts. |
| Journey Template | Generic Pranava lifecycle each Project inherits. Durations, SLAs, wording are data — never code. |
| SLA ≠ Plan | SLA is allowed service time. Plan is when this record should occur. Both are calculated independently. |
| Digital Home Passport | Equipment, serials, warranties, paint/tile codes, service history — lives with the unit forever. |
| Five feature tests | Trust? Less chasing? Accountability? Earlier prediction? Margin? If none, do not build. |

---

## 6. What 2.0 must achieve

| Outcome | Meaning |
|---|---|
| Experience | Premium, proactive, predictable journey from booking through post-handover. |
| Efficiency | Employees work from prioritized actions instead of searching modules and chasing colleagues. |
| Openness | Customers and stakeholders see the right information, at the right level, with ownership and timelines. |
| Control | Every promise, approval, SLA, blocker and exception is traceable. |
| Profitability | Collection risk, concessions, rework, delay cost and cost-to-serve are measurable by customer and unit. |
| Learning | Snags, delays, complaints and leakage feed root-cause analysis and process improvement. |
...
If none is yes, do not build. For unit customisations add a sixth: does this preserve the permanent as-built truth while protecting schedule and margin?

HomeFlow is the **orchestration brain**. It should not duplicate deep construction management, accounting, banking or document-storage when those systems already exist. It consumes authoritative data, converts it into lifecycle state, triggers actions, exposes controlled transparency, and keeps the customer/unit audit trail. If those systems do not exist, HomeFlow still operates natively.

---

## 7. Anatomy — twins and data model

Do not attach all commercial and lifecycle data directly to the customer or the unit. Attach it to the Booking wherever the fact belongs to a particular customer–unit ownership relationship.

### Four independent persistents (and related)

| Entity | Purpose | Must persist independently? |
|---|---|---|
...
Live physical truth of a unit — **even before it is sold.**
...
Relationship record for the family / entity.
...
### Project is the operating partition
...
```
0 Pre-sales → 1 Booking → 2 Funding → 3 Agreement → 4 Construction
            → 5 Collections → 6 Pre-reg → 7 Registration
            → 8 Pre-HO → 9 Handover → 10 Post-HO
```

Parallel streams may skip numbered order unless a gate blocks. Source: spec §5 / §34.2.

| Stage | Primary outcome | Typical parallel streams / exit logic |
|---|---|---|
...
### East Crest is configuration, not code
...
| Current East Crest concept | Generic HomeFlow mapping | Optimization in HomeFlow |
|---|---|---|
...
## 9. Changeability and Change Requests
...
**Operating principle.** A Change Request may be raised at almost any point. Gates govern feasibility, approval, execution, commercial treatment and authority — they should not suppress request creation. Sales/CRM **read** gates. Project/Design **own** physics and rules. Safety/statutory hard gates are never overridden.
...
### How a gate is derived — examples

| Physical / procurement event | Illustrative automated gate |
|---|---|
...
### Requirement-to-unit matching (sales guidance, not engineering approval)
...
Not a comment or snag. Rejected, Withdrawn and Cancelled remain auditable side states. Source: spec §8.7.

```
Requested → Feasibility → Costing → Internal appr. → Customer appr.
         → Payment → Approved → Released → In progress
         → Ready QA → QA verified → Accepted → As-built
```

### Core control model

| Control | HomeFlow requirement | Why it matters |
|---|---|---|
...
### 8.2 Documents & Legal Document Factory

Repository work becomes an intelligent pipeline.
...
Move from reporting outstanding to predicting cash realization.
...
Prevent future overdue by managing disbursement readiness.
...
Exception-driven and traceable.
...
Readiness explicit and forecastable.
...
### 8.7 Customer Change Requests & Unit Customisations

Controlled variations to the Unit Twin. Full engine is in [§9](#9-changeability-and-change-requests).
...
### 8.8 Unit Readiness & QA/QC

Replace subjective percentages with evidence-based completion.
...
Discovery → verified closure → root cause.
...
A gated readiness event — not an appointment alone.
...
### 8.11 Customer Commitments — Promise Ledger

Permanent ledger of every promise.
...
One auditable history. No internal chatter leaked.
...
Exception-management engine, not a noise channel.
...
### 8.14 Post-Handover / Warranty / Service

The relationship continues after keys.
...
## 11. Operate — scores, actions, My Day, Control Tower

### Five scores — each answers a different question
...
| Score | Question answered | Illustrative dimensions |
|---|---|---|
...
The operating backbone. Every task, snag, approval, gap, delay and AI rec normalizes into one Action object.
...
### Role workspaces — one question each
...
| Stakeholder | HomeFlow must answer |
|---|---|
...
Management should see **five problems that need intervention**, not fifty charts. The five interventions are system-generated, ranked, with owner, rupee/customer impact and the decision required. The five: **customer, cash, handover, reputation, margin.**
...
### Forecast line sources

| Source type | In committed post-sales forecast? |
|---|---|
| Contractual Due | Yes |
...
### Current-month plan stack

The UI must compare five views (illustrative shape, not production numbers): Plan · Month-start Forecast · Latest Forecast · Actual-to-date · Projected EOM. Source: spec §31.4.

### Project cash-flow math (every period)

Opening outstanding → demands due in period → overdue at start → expected contractual collection → expected overdue recovery → expected loan disbursement → expected rescheduled amount → total expected receipts → actual receipts → forecast variance → closing expected outstanding + confidence.

Project total equals the sum of its forecast lines. Portfolio total equals the sum of authorized Projects, subject to explicit elimination rules.

### Profitability and margin protection

| Economic object | Examples |
|---|---|
...
Hierarchy: Portfolio → Project → Phase/Tower/Block → Unit. Org view: Portfolio → Project Team / Shared Team → Role → Owner. **Project is a mandatory dimension.**
...
AI sits on disciplined workflow — **not a chatbot first.**

### Engines

| Engine | Purpose | Example output |
|---|---|---|
...
### Copilot questions by role (after the engines exist)
...
**AI limits.** AI may summarize redlines, suggest a known approved clause, flag anomalies, extract data and draft communications. Final legal wording comes from the approved library or an explicit Legal-approved deviation. AI cannot autonomously approve legal deviations or auto-send consequential customer communication.

### My Pranava Home — the customer never sees the OS
...
## 14. Ship — roadmap and what not to build

Architecture first, copilots last. Source: spec §24. P0 = 9 capabilities (0–90d). P1 = 8. P2 = 4.

### Priority roadmap

| Priority | Capability | Why now | Indicative outcome |
|---|---|---|---|
...
### Foundation acceptance tests (§26)

1. Every active booking resolves to exactly one current unit and one or more valid applicants/customers.
2. Unit history remains intact when booking/customer changes.
3. Every actionable record appears in the universal action engine with owner, SLA and evidence requirement.
4. Every readiness score is explainable down to component/blocker level.
5. Hard handover gates cannot be bypassed without configured authority and audit reason.
6. Every customer-facing commitment has owner, due date, status, dependencies and evidence.
7. Every overdue collection has a structured reason and next action.
8. Management can identify the top five portfolio interventions without navigating multiple reports.
9. Every financial and operational record traces Portfolio → Project → Unit → Booking/Customer without duplicate manual project tagging.
10. A user assigned to one Project sees that Project by default; a shared-team user switches only among authorized Projects and keeps correct task/escalation context.
11. Management can view last-period actual collections, current-period actual-to-date, next-month forecast and 30/60/90-day forecast by Project and portfolio.
12. Every forecast value drills to contributing Booking/Unit records and shows source, expected date, probability/confidence and last update.
13. The system retains forecast snapshots and calculates forecast-to-actual variance without overwriting prior forecasts.
14. Customer-facing information never exposes internal notes or unapproved assumptions.
15. Critical workflows and edits have a complete audit trail.
16. Every customer-requested unit change has a unique request ID, structured scope, impact assessment, approval state and auditable disposition.
17. No change can be released to Site before configured feasibility, commercial approval, customer acceptance and payment gates are satisfied.
18. Site, QA and Procurement can identify the current released drawing/specification revision; superseded revisions are visibly locked.
19. Every completed customisation updates the permanent Unit Twin / as-built record and preserves variation economics.
20. Every saleable unit has a current physical-progress record and an explainable changeability state by configured change category.
21. Sales and CRM can view but cannot directly edit physical progress or technical gate states.
22. When a configured construction/procurement event changes, affected gates are automatically re-evaluated and the event is audited.
23. Sales can filter/compare available units by changeability and see source timestamp/freshness before discussing personalisation.
24. A prospect’s Must Have / Preferred personalisation requirements can be compared with available units without implying technical approval.
25. Closing gates display an expected expiry date/event; stale forecasts are clearly identified.
26. Any Change Window Hold is time-bound, Project-approved when construction is affected, automatically expires, and leaves a complete audit trail.
27. Hard Closed gates cannot be reopened by Sales, CRM or ordinary management override.
28. Unit-level exceptions remain possible when actual unit progress differs from tower/floor bulk progress, with reason and audit.
29. Approved users can generate an AOS, Sale Deed or Lease Agreement only from an approved template version valid for the Project/transaction context.
30. Generated documents auto-populate authoritative fields and prevent release when mandatory source data is missing or inconsistent.
31. Every generated document records template version, data snapshot, clause selections, overrides/deviations, creator, reviewers, approvals, timestamps and final executed artifact.
32. Any manual change to a protected commercial/legal field requires configured authority and an auditable reason; ordinary users cannot silently edit generated legal text or values.
33. Legal can compare revisions and identify substantive clause/value changes before approval; superseded drafts remain accessible but cannot be mistaken for the current executable version.
34. Final executed/registered documents are locked, checksum/version identified, and visible from Project, Unit, Booking and Customer records according to permissions.
...
## 15. Emergent build — implementation contracts
...
### §30 Unit Progress, Configuration and Changeability
...
### §31 Project architecture and collections forecasting

Build Project as a platform-level partition and analytical dimension, not an optional filter. All new features must work at Unit/Booking detail and roll up to Project and portfolio.

Canonical hierarchy: **Portfolio → Project → Phase/Tower/Block/Cluster → Floor → Unit → Booking → Customer/Applicant.**
...
### §33 Project-owned live unit status for Sales and CRM

**R1/R2 implementation rule.** Build the Project Unit Status Console and Unit Twin so Project/Construction users operate them directly in HomeFlow. Generic integration primitives may be added later. Sales/CRM must never directly edit construction progress, QA truth, technical feasibility or hard changeability gates.

Closed loop: HomeFlow Project update → Unit Digital Twin → component/trade state → changeability rule evaluation → Sales/CRM inventory visibility → affected prospect/customer warnings → Change Request routing.

Example: electrical first-fix starts for Floors 8–12 → mapped electrical gates move Open to Conditional/Exception; Sales sees it immediately; prospects with electrical Must Have are flagged.
...
### §34 Configurable Project Journey and Universal Timeline/SLA Engine
...
### §35.1 Additional mandatory audit events
...
## 16. Reference — statuses, events, screens, integrations, policy

### Appendix A — core status taxonomy
...
### Appendix B — minimum event log

Every consequential event that must be logged:
...
### Integration blueprint (§18) — optional, system-agnostic

| System / channel | HomeFlow relationship |
|---|---|
...
### Admin / Policy Studio (§21) — everything configurable
...
### Data quality, audit and governance (§22)
...
*End of structured HomeFlow OS spec. Interactive copy: [design spec canvas](../canvases/homeflow-2-design-spec.canvas.tsx). PDF source of truth: `docs/Pranava_HomeFlow_2.0_Full_Design_Spec_v8.pdf`.*
