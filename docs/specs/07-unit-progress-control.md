...
## Rules
1. Only roles with WRITE on module `unit_progress` (`SITE`, `QA`, `MANAGEMENT`, `SUPER_ADMIN`) may write; Sales/CRM/Customisation get `forbidden` — at the authorize layer **and** a handler-level guard (defence in depth, p44 §33.6 t3, p35 §30.5 t10).
