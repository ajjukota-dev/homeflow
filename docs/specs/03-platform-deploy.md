...
| AWS naming | **This account already holds an unrelated project tagged `Project=pranava-portal`** (EC2 `pranava-test`, S3 `pranava-fmwork-test-975050032697`, IAM `pranava-portal-ec2`/`pranava-test-ec2-role`) — someone else's/earlier work. Every HomeFlow resource is tagged `Project=homeflow` and named with a `homeflow-` prefix (`homeflow-api` ECR/App Runner, `homeflow-db` RDS, `homeflow-<purpose>-975050032697` S3, `homeflow-apprunner-role` IAM, `homeflow-monthly-budget`) — never `pranava-*`, never touching or referencing the existing `pranava-portal` resources. |
