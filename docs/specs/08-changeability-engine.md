...
`08.change_categories` registered against `change_category` in the generic draft/publish/history envelope — a plain lookup table, distinct from `08.change_gate_rule_studio`'s own bespoke DRAFT/PUBLISHED/RETIRED versioning in `changeability/core.ts`, which this did not touch. Zero new frontend code, same shape as `10.action_types`. `08.gate_expiry_sources` stays `built: false` — noted as needing its own look (it may turn out to be a documentation-only duplicate of the already-shipped Change Gate Rule Studio, not new work) rather than folded into this batch. Full detail (the cross-spec batch this shipped in, plus a real `GenericTableEditor` bug found and fixed while verifying it live) is in `TODO.md` §9 and `docs/demo/run-log.md`'s 2026-09-06 "Studio registry-only batch" entry.
