...
/** H3 — materialize the booking's payment plan as Demand rows. Idempotent. Emits demand.raised
 *  (02 Appendix B) for every demand whose trigger has already fired (a scheduled demand is
 *  raised later, by raiseDemandsForUnit, when its trigger actually fires).
...
export async function setupFunding(bookingId: string, ctx: Ctx, tx?: DbLike) {
  return withTx(tx, async (t) => {
    ...
    if (!planId) {
      const plan = await t.query<{ id: string }>(
      ...
      await t.query(`UPDATE booking SET payment_plan_id = $1 WHERE id = $2`, [planId, bookingId]);
    }

    const ms = await t.query<{
    ...
    const progress = await t.query<{ component_code: string; state_code: ProgressState }>(
    ...
    return listDemands(bookingId, t);
  });
}
    ...
    for (const d of demands) {
      ...
      if (progressAtLeast(current, min)) {
        ...
        await withTx(undefined, async (t) => {
          await t.query(`UPDATE demand SET status = 'due', due_date = $1 WHERE id = $2`, [dueDate, d.id]);
          await appendEvent(t, {
            type: "demand.raised",
            entity_type: "demand",
            entity_id: d.id,
