...
describe("project-site master data creation", () => {
  ...
  it("creates a project", async () => {
    const p = await createProject({ code: "westpark", name: "West Park" });
  ...
  it("creates a unit that immediately has derived gates + is available", async () => {
    const p = await createProject({ code: "northvale", name: "North Vale" });
    const u = await createUnit(p.id, { unit_number: "N-101", unit_type: "2BHK", facing: "East" });
    ...
    const list = await listUnits(p.id);
    expect(list.length).toBe(1);
    expect(list[0].unit_number).toBe("N-101");
  });

  it("rejects a unit with no number", async () => {
    const p = await createProject({ code: "eastvale", name: "East Vale" });
    await expect(createUnit(p.id, { unit_number: "", unit_type: "3BHK", facing: "East" })).rejects.toThrow();
  });
});
