 ...
 *  automatic trigger. Invite/respond are CRM-only server-side (`CRM_UPDATE_ROLES`); FM sees this
 *  tab read-only. */
export function AdvocacyPanel({ bookingId, checkIns, roles }: { bookingId: string; checkIns: CheckInRow[]; roles: string[] }) {
  const canPublish = roles.some((r) => CRM_ROLES.includes(r));
