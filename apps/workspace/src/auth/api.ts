...
// (Vite proxies /api → the local API), so no explicit credentials mode needed.
...
export const authApi = {
  login: (email: string, password: string) => post<{ actor: Actor }>("/api/auth/login", { email, password }),
  ...
  acceptInvite: (token: string, password: string) => post<{ actor: Actor }>("/api/auth/invite/accept", { token, password }),
};
