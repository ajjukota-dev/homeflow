...
// The Portal leg lives in apps/my-pranava-home/e2e (Ananya customer@ + Rohan rohan@).
const sizes = [
...
for (const s of sizes) {
  test(`Sale-to-handover journey @ ${s.name}`, async ({ page }) => {
    ...
    await expect(main.getByRole("heading", { name: "Inventory" })).toBeVisible();
    await expect(main.getByText("V110")).toBeVisible();
    ...
    await expect(main.getByRole("heading", { name: "CRM · Relationship" })).toBeVisible();
    await expect(main.getByText("Villa V110")).toBeVisible();
    ...
    await expect(main.getByLabel("Loading collections")).toHaveCount(0);
    await expect(main.getByText("Karthik Iyer").first()).toBeVisible();
    await expect(main.getByText("Invalid Date")).toHaveCount(0);
    ...
    await expect(main.getByRole("heading", { name: "Document factory" })).toBeVisible();
    await expect(main.getByText("Rohan Desai")).toBeVisible();
    ...
    await expect(main.getByText("Keys issued").first()).toBeVisible();
    ...
    await expect(main.getByRole("heading", { name: "After keys" })).toBeVisible();
    await expect(main.getByRole("cell", { name: "V113" })).toBeVisible();
    await page.screenshot({ path: shot(`journey-after-keys-${s.name}`), fullPage: true });

    expect(errors, "no pageerror on sale-to-handover").toEqual([]);
  });
}
