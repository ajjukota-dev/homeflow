...
| R0.6 | **authorize()/requireRole() wired into every route** — all ~49 routes across `server.ts`/`routes-lifecycle.ts`/`routes-model.ts` now check the permission_matrix (`authorize()`, ~34 routes) or a direct role allow-list (`requireRole()`, ~15 pre-booking master-data routes that predate the matrix — project/unit/hierarchy creation, sales-inventory browsing, booking-lifecycle admin) | 🟨 | PR #15 open — 248/248 API tests green, `route-gates.test.ts` proves wrong-role rejection + allowed-role pass on a representative handler per gate type; background agent verifying frontend/e2e before merge |
