...
**Position:** R0 complete (5/5 merged), R0.5 (schema reconciliation) done, R0.6 (authorization) merged, R1 in progress (Roadmap page) · **Live URL:** https://we947t2rq2.ap-south-1.awsapprunner.com (`/health` → `{"ok":true,"db":true}` against real RDS) · **Last deploy:** 2026-09-05 R0-03 merge · **Last updated:** 2026-09-05 14:50 IST
...
| R1 | screen migration + journeys + Roadmap page + demo seed | 🟨 | Roadmap page (PR #16) — Management > Roadmap lists the 27 not-yet-merged specs from `apps/workspace/src/roadmap.json`, generated by reading each spec's title + PDF refs, keyed to the wave column above; first real `@homeflow/ui` consumer inside `apps/workspace` (`PageHeader`/`Badge`), proving the already-wired tokens/preset plumbing works. Demo seed for 3 products (PR #17) — checked first and found the premise wrong: a second demo project (`p_meadows`, MIXED, villa+plot units) already existed from an earlier session (`seed-canonical.ts`); only APARTMENT was missing, so added a 2-unit apartment tower to that same project (genuinely MIXED) rather than a redundant 4th project. Screen migration itself not started: local `src/ui/Button`/`Card`/`Segmented` primitives don't map 1:1 onto `@homeflow/ui`'s (different variant names, no `Card` equivalent at all) — needs a mapping decision before the ~22 workspace pages move. `e2e/journeys/sale-to-handover.spec.ts` (PR #18) — read-only walk of Sales → CRM → Collections → Legal → QA → Handover at 1440/768/375 (Portal leg already covered by `apps/my-pranava-home/e2e/auth.spec.ts`); hit the shared-DB flakiness for real while writing it — `admin-model.spec.ts`'s customer-merge test had permanently re-pointed V110 from Karthik Iyer to Meera Krishnan on the persisted dev DB from an earlier full-suite run, so the CRM-leg assertion was rewritten to key off the unit number (never re-pointed) instead of the owning customer's name. |
